...
### v0.5.6 (2026-04-02) — 6가지 근본 원인 해결

**원인 A (COM↔XML 단절):** XML 라우팅 완전 제거 → COM 우선 검색
**원인 F (에러 대화상자):** 파일 경로/권한/잠금 사전 검증 → 한글 얼럿 방지
- validate_file_path: 디렉토리 존재, 쓰기 권한, 파일 잠금 확인
- error_type 추가: permission_denied, invalid_path

### v0.5.5 (2026-04-02) — COM 우선 + 표 중복 감지

- find_replace/text_search 등 5개 도구: XML 분기 제거 → COM 직행
- 표 스캔 중복 감지: 이전 표와 동일 데이터면 조기 종료 (50 팬텀 방지)

### v0.5.4 (2026-04-01) — 4회 연속 FAIL 근본 수정

- get_tables: MovePos(2) 커서 초기화 + 예외 로깅
- find_and_append: MovePos(2) + XML 문단 연결 검색 리팩토링
- insert_textbox: HShapeObject 좌표 지원
- 머리글 성능: style 조건 최적화

### v0.5.3 (2026-04-01) — 3차 테스트 반영 + 하네스 기반 고도화

**버그 수정:**
- `find_and_append` "not_found" → COM fallback 전 자동 save
- `get_tables` 0 반환 → 분석 전 자동 save
- `find_replace_multi` 첫 시도 실패 → save 후 200ms 대기
- `col_widths` 합계 초과 → 자동 축소 + 경고 (PDF 열 깨짐 방지)
- 임시 PDF 자동 정리

**신규 기능:**
- 구조화된 에러 응답: `error_type` + `guide` (5가지 분류)
- 머리글/바닥글 스타일: `font_size`/`bold`/`align`/`color` 파라미터
- 검색 대소문자 무시: `case_sensitive=false` 옵션

### v0.5.2 (2026-03-29) — PDF/XML 동기화 수정
- PDF 빈 화면 → export 전 자동 save
- XML 엔진 5곳에 save 선행 (COM↔XML 불일치 해결)
- DOCX/HTML 내보내기 즉시 "not_supported" 반환

### v0.5.1 (2026-03-25) — esbuild 번들, COM 대화상자
- esbuild 단일 번들 — node_modules 불필요
- COM 대화상자 자동 차단 (머리글 336초→10초)
- Agent 트리거 11개 추가
